...
> ⚠️ Этот билд не нотаризован — при первом запуске macOS его заблокирует.
> Откройте **Системные настройки → Конфиденциальность и безопасность**, найдите
> сообщение о заблокированном Stampo и нажмите **«Всё равно открыть»**. Сделать
> это нужно один раз.
>
> Привычный обход через правый клик → «Открыть» в macOS 15 больше не работает:
> Apple его убрала, а Stampo требует macOS 15.7 и новее.
