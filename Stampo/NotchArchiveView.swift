...
final class ArchiveDragShimView: NSView, NSDraggingSource {
    ...
    private func updateHoverState() {
        ...
        // (The badge takes its own clicks by sitting above this view in the
        // SwiftUI overlay order; this view yields no rect to it — see the
        // note on `mouseDown`.)
        let bleed: CGFloat = 3
    ...
    /// No `hitTest` override: the delete badge is a SwiftUI overlay placed
    /// after this view and wins the click on its own, measured on the real
    /// hierarchy rather than assumed.
    ///
    /// There used to be one, stepping out of a square at the top-right corner
    /// so the badge could be reached. It stepped out of the *bottom*-right one:
    /// `hitTest` is given its point in the superview's coordinates, and the
    /// superview SwiftUI wraps a representable in is not flipped while this view
    /// is — so the rect was mirrored, and the corner it actually freed had
    /// nothing in it but cell. A click just under the badge reached neither and
